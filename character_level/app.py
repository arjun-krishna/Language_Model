...
@desc: Language Model (character-level)
"""
...
def main(_):
  ...
  with tf.Session() as sess:
    ...
    def generate_text():
      ...
      char = np.random.choice(dm.vocab_size, (1,1))
      ...
      for i in range(FLAGS.gen_len):
        ...
        feed_dict[inputs] = char

        pred, state = sess.run([sampler.prediction, sampler.final_state], feed_dict=feed_dict)

        char_id = np.argmax(pred[0][0])

        generated_txt += dm.id2char[char_id]

        char = np.array([[char_id]])
